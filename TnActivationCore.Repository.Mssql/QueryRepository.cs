...
namespace TnActivationCore.Repository.Mssql
{
        ...
        where TDbContext : DbContext
    {
        ...
        public Task<List<T>> GetListAsync<T>(Expression<Func<T, bool>> condition,
            CancellationToken cancellationToken = default)
            where T : class
        {
        ...
        public async Task<List<TEntity>> GetRightJoinedListAsync<TEntity>(
            Expression<Func<TEntity, bool>> condition,
            Func<IQueryable<TEntity>, IQueryable<object>> joinQuery,
            CancellationToken cancellationToken = default)
            where TEntity : class
        {
            ...
            var joinedQueryResult = joinQuery(query);

            var joinedList = await query
                .GroupJoin(
                    joinedQueryResult,
                    entity => entity,
                    joinEntity => joinEntity,
                    (entity, joinEntities) => new { Entity = entity, JoinEntities = joinEntities })
                .SelectMany(
                    x => x.JoinEntities.DefaultIfEmpty(),
                    (x, joinEntity) => new { Entity = x.Entity, JoinEntity = joinEntity })
                .ToListAsync(cancellationToken);

            return joinedList.Select(x => x.Entity).ToList();
        }
        ...
        public Task<List<T>> GetListAsync<T>(Specification<T> specification,
            CancellationToken cancellationToken = default)
            where T : class
        {
            ...
            where T : class
        {
            ...
            PaginatedList<T> paginatedList =
                await _dbContext.Set<T>().ToPaginatedListAsync(specification, cancellationToken);
        ...
        public async Task<List<T>> GetFromRawSqlAsync<T>(string sql, object parameter,
            CancellationToken cancellationToken = default)
        {
            if (string.IsNullOrWhiteSpace(sql))
            {
        ...
        public async Task<List<T>> GetFromRawSqlAsync<T>(string sql, IEnumerable<DbParameter> parameters,
            CancellationToken cancellationToken = default)
        {
            if (string.IsNullOrWhiteSpace(sql))
            {
        ...
        public async Task<List<T>> GetFromRawSqlAsync<T>(string sql, IEnumerable<object> parameters,
            CancellationToken cancellationToken = default)
        {
            if (string.IsNullOrWhiteSpace(sql))
            {
