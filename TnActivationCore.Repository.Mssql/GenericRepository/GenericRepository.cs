...
namespace TnActivationCore.Repository.Mssql.GenericRepository
{
    internal class GenericRepository<TDbContext> : QueryRepository<TDbContext>, IGenericRepository, IGenericRepository<TDbContext>
        where TDbContext : DbContext
    {
            ...
            where TEntity : class
        {
            ...
            EntityEntry<TEntity> trackedEntity =
                _dbContext.ChangeTracker.Entries<TEntity>().FirstOrDefault(x => x.Entity == entity);
            ...
            if (trackedEntity == null)
            {
                ...
                if (entityType == null)
                {
                    throw new InvalidOperationException(
                        $"{typeof(TEntity).Name} is not part of EF Core DbContext model");
                }
        ...
        public async Task<int> DeleteAsync<TEntity, TPKeyType>(TPKeyType id, CancellationToken cancellationToken = default)
            where TEntity : class 
        {
            var entity = await this.GetByIdAsync<TEntity>(id, cancellationToken);
            if (entity == null)
            {
                throw new ArgumentNullException(nameof(entity));
            }
        ...
        public Task<int> ExecuteSqlCommandAsync(string sql, IEnumerable<object> parameters,
            CancellationToken cancellationToken = default)
        {
